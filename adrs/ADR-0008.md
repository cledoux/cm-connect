...
1. **Artifact Exclusion and Deferred Reset in Patch Extraction:**
   - Working tree diff extraction (`git add -N` and `git diff HEAD`) in
     `cmpatch.ExtractPatch` MUST apply magic pathspec exclusions
     (`:!.cm_project`, `:!.codemender`, `:!.codemender-out`,
     `:!change_envelope.json`, `:!*creds*.json`, `:!*.diff`, `:!*.tmp`), execute
     deferred `git reset HEAD -- .` cleanup, and pass matching `-x` exclusions
     during fallback directory diffing to ensure runtime state, credentials, and
     ephemeral output files are never included in remediation patches.
