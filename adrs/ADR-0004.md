...
1. **Credential Compartmentalization:**
   ...
   - The write-scoped repository token (`GITHUB_TOKEN` or PR comment token) MUST
     NOT be mounted or injected into the container environment executing
...
1. **Host-Side Zero-Setup Publisher (`publish_comments.py`):**
   ...
   - Supports a Two-Tier commenting architecture:
     - **Tier 1 (`--mode=summary`):** Ingests `findings.json` to publish an
       executive scan summary issue comment and `$GITHUB_STEP_SUMMARY` detailing
       finding metrics and collapsible threat analysis.
     - **Tier 2 (`--mode=inline`):** Ingests `change_envelope.json` to publish
       lightweight 1-click apply review suggestions.
     - **Advisory Mode (`--advisory`):** Ingests out-of-diff findings to post
