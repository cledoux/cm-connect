...
- **Direct Repository Anchoring & Context Awareness**: The staged diff file MUST
  be located in the repository root (`/workspace/pull-request.diff`) so that
  CodeMender anchors its scan workspace context to `/workspace`, allowing the
  LLM reasoning engine to inspect changed repository source files in-place.
- **`git clean` Immunity via Disabled VCS Reset**: The runner MUST disable
  CodeMender's pre-scan VCS reset by calling `DisableReset()`
  (`vcs.commands.reset: "true"`) in `$HOME/.codemender/config.yaml`,
  guaranteeing the staged diff cannot be deleted during scan execution.
- **Zero Persistent Workspace Pollution**: The staged diff file MUST be
  ephemerally deleted upon command completion, leaving zero untracked files in
  `/workspace` and protecting subsequent `fix` patch extractions from untracked
  file pollution.
...
- **Minimal High-Density Context Prompt with Grounding**: Injects a concise
  ...
  to ground the reasoning model without prompt bloat, while appending any
...
- **Option 1: Dedicated Top-Level `find-diff` Subcommand with Repository-Root
  Staging, Disabled VCS Reset, and AST Config Mutation (Chosen)** —
  `cm-runner find-diff <git-diff-args...> [-- <cm-flags...>]` executes
  `git diff` inside `/workspace`, saves the patch to
  `/workspace/pull-request.diff`, mutates `$HOME/.codemender/config.yaml` to
  include `.diff` and disable VCS reset (`vcs.commands.reset: "true"`), injects
  the consolidated context flag, executes the two-phase scan/report sequence
  (`cm find /workspace/pull-request.diff` -> `cm report --format=json`), and
  cleans up the staged diff file upon exit.
...
Repository-Root Staging, Disabled VCS Reset, and AST Config Mutation**.

### Justification

1. **Direct Repository Anchoring & Context Awareness**: By staging the diff at
   `/workspace/pull-request.diff`, CodeMender anchors its scan workspace
   directly at the repository root `/workspace`. This allows the LLM reasoning
   agent to inspect changed source files in-place with full repository context.
1. **Disabled VCS Reset Protection**: By mutating
   `$HOME/.codemender/config.yaml` via `DisableReset()` to set
   `vcs.commands.reset: "true"`, `cm find` does not execute
   `git checkout HEAD -- . && git clean -fd` during diff scanning, preserving
   the staged diff file and eliminating spurious VCS reset errors.
...
1. **Clean Ephemeral Teardown**: Automatically removes
   `/workspace/pull-request.diff` upon completion, ensuring zero untracked
   residue in the repository for subsequent operations.
...
flowchart TD
    ...
    subgraph Config["2. Dynamic Config Update (pkg/cmconfig)"]
        Mutate["Mutate $HOME/.codemender/config.yaml<br>1. Append '.diff' to scan.extensions.include<br>2. Set vcs.commands.reset to 'true'"]
    end
    ...
    subgraph DiffGen["3. Git Diff Generation"]
        ...
        WriteScratch["Write to /workspace/pull-request.diff"]
    ...
    subgraph Execution["4. Two-Phase Scan Pipeline (pkg/cmrunner)"]
        PromptPrep["Consolidate Context Flag:<br>--context='You are evaluating a change request. The target is the unified diff containing the change. You are executing in the root directory of the repository and so have access to any repo files you need for context. [user context]'"]
        Phase1["Phase 1: cm find /workspace/pull-request.diff -y --context='...' [flags]<br>(stderr progress)"]
        Phase2["Phase 2: cm report --format=json<br>(stdout JSON payload)"]
        Cleanup["rm -f /workspace/pull-request.diff"]
    end
...
- **Base Context String**:
  `"You are evaluating a change request. The target is the unified diff containing the change. You are executing in the root directory of the repository and so have access to any repo files you need for context."`
- **Context Merging Rule**: If the caller passes `-c` or `--context` in
  ...
  `--context="You are evaluating a change request. The target is the unified diff containing the change. You are executing in the root directory of the repository and so have access to any repo files you need for context. Check SQL injection"`
...
- **Good, because** stages the diff inside the repository root, allowing
  CodeMender to anchor to `/workspace` and inspect changed repository files.
- **Good, because** disables the VCS reset command during diff scanning,
  preventing `git clean` from deleting the staged diff.
- **Good, because** performs clean post-scan teardown, leaving zero untracked
...
- **Good, because** dynamically enables `.diff` scanning via reusable
  `pkg/cmconfig`.
- **Good, because** repository grounding context gives CodeMender unambiguous
  location awareness.
...
digraph FindDiffArchitecture {
  ...
  subgraph cluster_container {
    ...
    WorkspaceDiff [label="Repository Diff Staging\n/workspace/pull-request.diff", style=filled, fillcolor="#fef7e0"];
    ConfigYAML [label="$HOME/.codemender/config.yaml\n(scan.extensions.include: [.rs, .diff]\nvcs.commands.reset: \"true\")", style=filled, fillcolor="#ceead6"];
    
    GitProc [label="git diff origin/main HEAD", shape=ellipse, style=filled, fillcolor="#d2e3fc"];
    FindProc [label="cm find /workspace/pull-request.diff -y -c \"...\"", shape=ellipse, style=filled, fillcolor="#d2e3fc"];
  ...
  GitProc -> WorkspaceDiff [label="2. writes diff"];
  Cmd -> ConfigYAML [label="3. mutates config"];
  WorkspaceDiff -> FindProc [label="4. Phase 1 scan"];
...
1. **Native Git Diff Extraction Test**:
   - Execute `cm-runner find-diff HEAD~1 HEAD` against a Git repository with
     known commits.
   - Assert `/workspace/pull-request.diff` is created inside `/workspace` and
     deleted upon exit.
...
1. **Git Error Discrimination Test**:
   ...
   - Assert process exits with code `2` and logs diagnostic error advising git
     revision validity.
1. **Dynamic `.diff` Extension & VCS Reset Mutation Test**:
   - Verify `$HOME/.codemender/config.yaml` contains `.diff` in
     `scan.extensions.include` and `vcs.commands.reset: "true"` after
     `find-diff` runs.
...
1. **Workspace Clean Teardown Test**:
   - Assert zero untracked files remain in `/workspace` after `find-diff`
     execution completes.
