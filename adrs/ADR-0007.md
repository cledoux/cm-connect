...
- **Direct Repository Anchoring & Context Awareness**: The staged diff file MUST
  be located in the repository root (`/workspace/.codemender-diff.diff`) so that
...
- **`git clean` Immunity via Disabled VCS Reset**: The runner MUST disable
  CodeMender's pre-scan VCS reset by mutating `vcs.commands.reset` to `"true"`
  in `$HOME/.codemender/config.yaml`, guaranteeing the staged diff cannot be
  deleted during scan execution.
...
- **Option 1: Dedicated Top-Level `find-diff` Subcommand with Repository-Root
  ...
  `/workspace/.codemender-diff.diff`, mutates `$HOME/.codemender/config.yaml` to
  include `.diff` and set `vcs.commands.reset` to `"true"`, injects the
  consolidated context flag, executes the two-phase scan/report sequence
  (`cm find /workspace/.codemender-diff.diff` -> `cm report --format=json`), and
...
1. **Direct Repository Anchoring & Context Awareness**: By staging the diff at
   `/workspace/.codemender-diff.diff`, CodeMender anchors its scan workspace
...
1. **Disabled VCS Reset Protection**: By mutating
   `$HOME/.codemender/config.yaml` to set `vcs.commands.reset: "true"`,
   `cm find` does not execute `git checkout HEAD -- . && git clean -fd` during
   diff scanning, preserving the staged diff file and eliminating spurious VCS
   reset errors.
...
1. **Clean Ephemeral Teardown**: Automatically removes
   `/workspace/.codemender-diff.diff` upon completion, ensuring zero untracked
...
flowchart TD
    ...
    subgraph DiffGen["3. Git Diff Generation"]
        ...
        WriteScratch["Write to /workspace/.codemender-diff.diff"]
    ...
    subgraph Execution["4. Two-Phase Scan Pipeline (pkg/cmrunner)"]
        PromptPrep["Consolidate Context Flag:<br>--context='The target is a Git unified diff for this repository. You are executing in the root directory of the repository. [user context]'"]
        Phase1["Phase 1: cm find /workspace/.codemender-diff.diff -y --context='...' [flags]<br>(stderr progress)"]
        Phase2["Phase 2: cm report --format=json<br>(stdout JSON payload)"]
        Cleanup["rm -f /workspace/.codemender-diff.diff"]
    end
...
- **Base Context String**:
  `"The target is a Git unified diff for this repository. You are executing in the root directory of the repository."`
- **Context Merging Rule**: If the caller passes `-c` or `--context` in
  ...
  `--context="The target is a Git unified diff for this repository. You are executing in the root directory of the repository. Check SQL injection"`
...
digraph FindDiffArchitecture {
  ...
  subgraph cluster_container {
    ...
    WorkspaceDiff [label="Repository Diff Staging\n/workspace/.codemender-diff.diff", style=filled, fillcolor="#fef7e0"];
    ...
    FindProc [label="cm find /workspace/.codemender-diff.diff -y -c \"...\"", shape=ellipse, style=filled, fillcolor="#d2e3fc"];
...
1. **Native Git Diff Extraction Test**:
   - Execute `cm-runner find-diff HEAD~1 HEAD` against a Git repository with
     known commits.
   - Assert `/workspace/.codemender-diff.diff` is created inside `/workspace`
