...
flowchart LR
    subgraph Host["Host / CI Orchestrator"]
        Workspace["Codebase (/workspace)<br>[rw volume]"]
        FindingsFile["Findings Artifact<br>(/workspace/findings.json)"]
        AuthSource["Auth Source<br>[ADC Mount, Access Token, or Creds JSON]"]
        JSONCollector["JSON / SARIF Consumer<br>[stdout pipe / jq]"]
        LogCollector["Log / Diagnostics Sink<br>[stderr pipe]"]
    end

    subgraph Container["Batch Runner Container (USER: codemender / UID 1000)"]
        GoRunner["Go Entrypoint Runner<br>(/usr/local/bin/cm-runner)"]
        SQLiteDB["SQLite State DB<br>(/home/codemender/.codemender/state.db)"]

        subgraph Stages["find Pipeline (Default: --format json)"]
            FindStage["cm find &lt;target&gt;<br>(Scan &amp; Detect)"]
            ReportStage["cm report --format json<br>(Format &amp; Query)"]
        end
    end
    ...
    Workspace -->|mounts rw| GoRunner
    AuthSource -->|injects auth| GoRunner

    GoRunner -->|executes Phase 1| FindStage
    FindStage -->|LLM reasoning| VertexAI
    FindStage -->|writes findings| SQLiteDB
    FindStage -->|logs to stderr| LogCollector

    GoRunner -->|executes Phase 2| ReportStage
    SQLiteDB -->|queries findings| ReportStage
    ReportStage -->|emits JSON on stdout| JSONCollector
    ReportStage -->|logs to stderr| LogCollector
```
