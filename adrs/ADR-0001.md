...
1. **Two-Phase Execution Multiplexing & Default Formatting (`cm-runner` Go
   binary):**
   - **Two-Phase Coordination (`find`):** When invoked with `find`, `cm-runner`
     first executes `/usr/local/bin/cm find <target>` with progress routed to
     `stderr`, and then executes `/usr/local/bin/cm report --format json` to
     query SQLite state and emit structured machine-readable findings to
     `stdout`.
   - **Machine-Readable by Default:** Defaults to `--format json` (or
     `-f json`), supporting format overrides (e.g. `sarif`, `table`, `html`,
     `md`).
   - **Idempotent Invocation:** If the first argument is `cm` (e.g.
     `docker run <image> cm find`), the wrapper normalizes it and dispatches the
     find pipeline.
   - **Direct Subcommand:** If invoked as `docker run <image> find`, it executes
     the find pipeline.
   - **Interactive Fallback:** If `shell` is invoked with an interactive
     pseudo-TTY, the runner launches `/bin/bash` in `/workspace`.
...
1. **Signal & Exit Code Forwarding:**
   ...
   - The container evaluates report findings to terminate with status code `0`
     (clean), `1` (findings detected), `2` (CLI/path error), or `> 2` (fatal
     error).
...
digraph G {
  ...
  subgraph cluster_container {
    ...
    SQLiteDB [label="SQLite State DB\n(/home/codemender/.codemender/state.db)"];
    
    subgraph cluster_stages {
      label="find Pipeline (Default: --format json)";
      style=dotted;
      color=darkgreen;

      FindStage [label="cm find <target>\n(Scan & Detect)"];
      ReportStage [label="cm report --format json\n(Format & Query)"];
    }
  }
  ...
  GoRunner -> FindStage [label="executes Phase 1"];
  FindStage -> VertexAI [label="LLM reasoning"];
  FindStage -> SQLiteDB [label="writes findings"];
  FindStage -> LogCollector [label="logs to stderr"];
  
  GoRunner -> ReportStage [label="executes Phase 2"];
  SQLiteDB -> ReportStage [label="queries findings"];
  ReportStage -> JSONCollector [label="emits JSON on stdout"];
  ReportStage -> LogCollector [label="logs to stderr"];
}
```
