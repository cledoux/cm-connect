...
func TestNormalize_SingleObject_PascalCase(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
	...
	outBytes, finding, err := Normalize(raw, ws)
	if err != nil {
...
func TestNormalize_SingleElementArray(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
	...
	if finding.FilePath != "main.go" || finding.Line != 10 || finding.VulnType != "CWE-79" {
		t.Errorf("unexpected finding content: %+v", finding)
	}
...
func TestNormalize_SnakeCaseAndCamelCaseFields(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
	...
	raw := []byte(`{
		"file_path": "api/handler.go",
		"line": 15,
		"title": "Hardcoded Secret",
		"message": "Do not hardcode credentials in code",
		"severity": "high",
		"vuln_id": "CWE-798",
	...
	_, finding, err := Normalize(raw, ws)
	if err != nil {
...
func TestNormalize_FilePrefixStripping(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
...
func TestNormalize_DefaultsAndFallbacks(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
	...
	t.Run("Line zero and negative defaults to 1", func(t *testing.T) {
		...
		_, findingNeg, err := Normalize(rawNeg, ws)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if findingNeg == nil {
	...
	t.Run("Empty message falls back to Snippet then Title", func(t *testing.T) {
		...
		_, finding1, err := Normalize(raw1, ws)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if finding1 == nil {
		...
		_, finding2, err := Normalize(raw2, ws)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if finding2 == nil {
	...
	t.Run("Severity defaults to HIGH and uppercases", func(t *testing.T) {
		...
		_, findingEmpty, err := Normalize(rawEmpty, ws)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if findingEmpty == nil {
		...
		_, findingLower, err := Normalize(rawLower, ws)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if findingLower == nil {
...
func TestNormalize_FieldAliases(t *testing.T) {
	...
	t.Run("filePath camelCase and start_line", func(t *testing.T) {
		raw := []byte(`{"filePath": "alias.go", "start_line": 20, "vulnType": "CWE-1", "analysis": "custom analysis"}`)
		_, f, err := Normalize(raw, ws)
		if err != nil || f == nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("Line and Message capitalized", func(t *testing.T) {
		raw := []byte(`{"FilePath": "alias.go", "Line": 30, "Message": "msg", "vulnId": "CWE-2"}`)
		_, f, err := Normalize(raw, ws)
		if err != nil || f == nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("line and message lowercase with vuln_type", func(t *testing.T) {
		raw := []byte(`{"file_path": "alias.go", "line": 40, "message": "msg2", "vuln_type": "CWE-3"}`)
		_, f, err := Normalize(raw, ws)
		if err != nil || f == nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if f.Line != 40 || f.Message != "msg2" || f.VulnType != "CWE-3" {
			t.Errorf("unexpected normalized finding: %+v", f)
		}
	})
}

func TestNormalize_WorkspaceValidation(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
	...
	for _, tc := range tests {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			_, _, err := Normalize([]byte(tc.rawJSON), ws)
...
func TestNormalize_ArrayCardinality(t *testing.T) {
	t.Parallel()
	ws := t.TempDir()
